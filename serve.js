...
async function handleRequest(event) {
  ...
  if (!response) {
    ...
    let response = new Response(body, {
      ...
      encodeBody: ext ? 'manual' : 'auto' // If the file is compressed, tell CF not to reencodde it.
    });
    ...
    response.headers.append('X-KimlikDAO-Cache', 'VAYAK');
  }
  return response;
}
...
addEventListener('fetch', event => event.respondWith(
  handleRequest(event).catch(hata)));
