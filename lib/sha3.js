 ...
 * @fileoverview
 */
import { Uint8denHexe } from '/lib/çevir';
...
const keccak256 = (veri) => {
  ...
  while (index < length) {
    for (i = start; index < length && i < BYTE_COUNT; ++index) {
      code = veri.charCodeAt(index);
      if (code < 0x80) {
  ...
  /** @type {string} */
  let hex = '';
  for (let j = 0; j < OUTPUT_BLOCKS;) {
    for (let i = 0; i < BLOCK_COUNT && j < OUTPUT_BLOCKS; ++i, ++j) {
      block = s[i];
      hex += Uint8denHexe[block & 0xFF] +
        Uint8denHexe[(block >> 8) & 0xFF] +
        Uint8denHexe[(block >> 16) & 0xFF] +
        Uint8denHexe[(block >> 24) & 0xFF];
    }
    if (j % BLOCK_COUNT === 0) {
      f(s);
      i = 0;
